...
var JotForm = {
    ...
    */
    ...
    handleIFrameHeight: function () {
        ...
        var calendar = this.calendarCheck()
        ...
        if (window.JFForm && window.JFForm.draftID) {
            ...
            formIDField.insert({
                after: new Element('input', {
                    ...
                    name: 'draftID',
                    id: "draftID"
                }).putValue(window.JFForm.draftID)
            });
        }
   ...
   */
        ...
        $$('.form-pagebreak').each(function (page, i) {
            ...
            section.select('.form-pagebreak-next').invoke('observe', 'click', function () { // When next button is clicked
                validatePaymentOnNextBtn(JotForm.payment).then(function(isPaymentValid) {
                    ...
                    JotForm.visitedPages[JotForm.currentSection.pagesIndex] = true;
    ...
    jumpToPage: function () {
        ...
        if(JotForm.doubleValidationFlag()) {
            ...
            JotForm.visitedPages[page] = true;
        }
    ...
    doubleValidationFlag: function () {
        ...
        if (getQuerystring('doubleValidation') === '1') {
            return true;
        }
        if (JotForm.payment) {
            return false;
        }
        if (testForms.indexOf(formID) > -1) {
            return true;
        }
    ...
    jumpToGivenPage: function (page) {
        ...
        JotForm.showFormSection(sections[page - 1], true);
    ...
    /**
     * Check if section touched by the form filler. (If any of the inputs filled, it means the form filler saw the section.)
     */
    isSectionTouched: function (section) {
        // TODO: find a better way to detect at least one input has value
        if (!section) return false;

        function isDefaultValue(el) {
            if(!el) return false;
            return JotForm.defaultValues[el.id] && JotForm.defaultValues[el.id] === el.value;
        }

        function filterFields(el) {
            var cont = JotForm.getContainer(el);
            var alwaysHidden = cont.hasClassName('always-hidden') || cont.hasClassName('form-field-hidden');

            return !alwaysHidden && !isDefaultValue();
        }

        var inputs = section.querySelectorAll('*:not(button)[id^="input_"], input[id^="first_"], input[id^="last_"], input[id^="lite_mode_"], .FITB').filter(filterFields);
        var filled = 0;

        for (var i = 0; i < inputs.length; i++) {
            var input = inputs[i];
            var field = JotForm.getContainer(input);
            var fieldType = field.getAttribute('data-type');

            if (fieldType === 'control_rating') {
                var starField = input.querySelector('input.form-star-rating-star');
                if (starField && starField.value) {
                    filled++;
                }
            } else if (fieldType === 'control_appointment') {
                var dateField = field.querySelector('input[name$="appointment[date]"]');
                if (dateField && dateField.value) {
                    filled++;
                }
            } else if (fieldType === 'control_inline') {
                var fitbInputs = field.querySelectorAll('input, select:not([id^="ampm"])');
                var fitbInputsFiltered = fitbInputs.filter(function (el) {
                    if (isDefaultValue(el)) {
                        return false;
                    }
                    if (['checkbox', 'radio'].include(el.type)) {
                        return el.checked;
                    }
                    return !!el.value;
                });

                if (fitbInputsFiltered.length > 0) {
                    filled++;
                }
            } else if (fieldType === 'control_payment') {
                if (!!(field.querySelector('input[type="checkbox"]:checked'))) {
                    filled++;
                }
            } else if (['checkbox', 'radio'].include(input.type)) {
                if (input.checked) {
                    filled++;
                }
            } else {
        ...
        return filled > 0 || JotForm.visitedPages[section.pagesIndex];
    },
    /**
  ...
  */
  handleSignSignatureInputs: function() {
    ...
    validateAll: function (form, scopeSelector) {
        if (JotForm.doubleValidationFlag()) { 
            // if has pagebreak and try to validate all without scopeSelector, avoid the validation. It's already validated section by section.
            var hasPageBreak = document.querySelector('.form-pagebreak');
            if (hasPageBreak && !scopeSelector) {
                return true;
            }
        }
        ...
        if (window.JCFServerCommon !== undefined) {
            ...
            widgetInputs.each(function (el) {
                if(JotForm.doubleValidationFlag()) {
                    ...
                    var alwaysHidden = JotForm.getContainer(el).hasClassName('always-hidden') || JotForm.getContainer(el).hasClassName('form-field-hidden');
                    ...
                    function _validateAll() {
                        if(JotForm.doubleValidationFlag()) {
                            ...
                            if (allSections.length === 1) {
                                return JotForm.validateAll(form);
                            }
                            
                            for (var i = 0; i < allSections.length; i++) {
                                if(JotForm.isSectionTouched(allSections[i])) {
                                    var result = JotForm.validateAll(form, allSections[i]);
                                    if (!result) {
                                        unvalidSections++;
                                    }
                                }
    ...
    */
    ...
    setFieldValidation: function (input) {
        ...
        input.validateInput = function (deep, dontShowMessage) { // dontShowMessage param will be passed and handled by cardForm
            ...
            if (JotForm.doubleValidationFlag()) {
                var alwaysHidden = JotForm.getContainer(input).hasClassName('always-hidden') || JotForm.getContainer(input).hasClassName('form-field-hidden');
                if (window.FORM_MODE === 'cardform') {
                    alwaysHidden = !JotForm.isVisible(input);
                }
            ...
            if (vals.include("required")) {
                ...
                if(JotForm.getContainer(input).getAttribute('data-type') == 'control_signature'){
                    ...
                    if (jQuery(pad).attr("data-required") === "true") {
                        ...
                        if (JotForm.doubleValidationFlag()) {
                            _isVisible = !(JotForm.getContainer(pad).hasClassName('always-hidden') || JotForm.getContainer(pad).hasClassName('form-field-hidden'));
                        }
                        ...
                        if (document.getElementsByName(otherInputName)[0]) {
                            ...
                            if (JotForm.payment && cont.getAttribute('data-type').match(JotForm.payment)) {
                                if (!$A(document.getElementsByName(input.name)).map(function (e) {
                                        ...
                                        if (JotForm.doubleValidationFlag()) {
                                            _isVisible = !(JotForm.getContainer(e).hasClassName('always-hidden') || JotForm.getContainer(e).hasClassName('form-field-hidden'));
                                        }
                                        if (_isVisible) {
